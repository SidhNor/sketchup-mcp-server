  ...
  class SocketServer
    ...
    def serialize_entity(entity)
      data = {
        ...
        material: entity.respond_to?(:material) ? material_name(entity.material) : nil,
      ...
      if entity.is_a?(Sketchup::Group) || entity.is_a?(Sketchup::ComponentInstance)
        data[:definition_name] = entity.is_a?(Sketchup::ComponentInstance) ? entity.definition&.name : nil
        data[:children_count] = entity_children_count(entity)
        data[:origin] = point_to_a(entity.transformation.origin) if entity.respond_to?(:transformation)
      end
    ...
    def entity_type_counts(entities)
      ...
      entities.each do |entity|
        counts[entity.typename.downcase] += 1
      end
      counts
    end
    ...
    def get_scene_info(params = {})
      ...
      entity_limit = (params["entity_limit"] || 25).to_i
      entity_limit = 1 if entity_limit < 1

      {
        ...
        counts: {
          ...
          by_type: entity_type_counts(model.entities.to_a)
        },
        bounds: bounds_to_h(model.bounds),
        entities: model.entities.to_a.first(entity_limit).map { |entity| serialize_entity(entity) }
      }
    end
    ...
    def list_entities(params = {})
      ...
      entities = entities.reject { |entity| entity.respond_to?(:hidden?) && entity.hidden? } unless include_hidden

      {
        success: true,
        count: entities.length,
        entities: entities.first(limit).map { |entity| serialize_entity(entity) }
      }
    end
    ...
    def get_selection
      ...
      selected_entities = selection.map { |entity| serialize_entity(entity) }
