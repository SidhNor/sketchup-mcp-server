...
module SU_MCP
  module Terrain
    # MTA-24 comparison-only CDT terrain candidate. Not production-wired.
    class CdtTerrainCandidateBackend # rubocop:disable Metrics/ClassLength
      ...
      def run(state:, feature_geometry:, base_tolerance:, max_point_budget:,
        ...
        normalized = normalize_input(state, feature_geometry, base_tolerance, max_point_budget)
        budget_status = budget_status_for(normalized, max_point_budget)
        constraints = if budget_status == 'max_point_budget_exceeded'
                        []
                      else
                        normalized.fetch(:segments)
                      end
        triangulation, mesh = triangulated_mesh(state, normalized, constraints)
        if budget_status == 'ok'
        ...
        budget_status = face_budget_status(mesh, budget_status, max_face_budget)
        budget_status = runtime_budget_status(started, budget_status, max_runtime_budget)
        metrics = metrics_for(
          state: state,
        ...
        result_row(state: state, feature_geometry: feature_geometry, normalized: normalized,
                   triangulation: triangulation, mesh: mesh, metrics: metrics,
                   budget_status: budget_status)
      ...
      def normalize_input(state, feature_geometry, base_tolerance, max_point_budget)
        ...
        segments = []
        segments.concat(protected_region_segments(feature_geometry))
        segments.concat(reference_segments(feature_geometry))
        {
      ...
      def refine_mesh_residuals(state:, normalized:, constraints:, base_tolerance:,
                                max_point_budget:, triangulation:, mesh:)
        limit = residual_refinement_point_limit(normalized, max_point_budget)
        normalized[:residual_refinement_safety_cap] = limit
        blocked_reason = blocked_residual_refinement_reason(
          state, mesh, base_tolerance, normalized, limit
        )
        if blocked_reason
        ...
        triangulation, mesh, stop_reason = run_residual_refinement_passes(
          state, normalized, constraints, base_tolerance, [triangulation, mesh], limit
        )
        stop_reason ||= residual_refinement_final_stop_reason(state, mesh, base_tolerance,
                                                              normalized, limit)
        normalized[:residual_refinement_stop_reason] = stop_reason
        add_residual_refinement_budget_limitation(normalized, limit)
        [triangulation, mesh]
      end

      def blocked_residual_refinement_reason(state, mesh, base_tolerance, normalized, limit)
        return 'disabled' unless residual_refinement_max_passes.positive?

        if normalized.fetch(:points).length >= limit
          normalized[:max_residual_excess] = max_residual_excess(
            state, mesh, base_tolerance, normalized
          )
          return 'safety_cap'
        end
      ...
      def run_residual_refinement_passes(state, normalized, constraints, base_tolerance,
                                         mesh_result, limit)
        triangulation, mesh = mesh_result
        stop_reason = nil
        residual_refinement_max_passes.times do
          samples = residual_samples(state, mesh, base_tolerance, normalized,
                                     residual_refinement_batch_size)
      ...
      def residual_samples(state, mesh, base_tolerance, normalized, limit)
      ...
      def max_residual_excess(state, mesh, base_tolerance, normalized)
        sample_residual_excess(residual_samples(state, mesh, base_tolerance, normalized, 1))
      end

      def residual_refinement_final_stop_reason(state, mesh, base_tolerance, normalized, limit)
        remaining = residual_samples(state, mesh, base_tolerance, normalized, 1)
      ...
      def add_residual_refinement_budget_limitation(normalized, _limit)
      ...
      def metrics_for(state:, feature_geometry:, normalized:, mesh:, triangulation:,
                      budget_status:, started:)
        ...
        coverage = triangulation.fetch(:constrainedEdgeCoverage)
        {
          ...
          maxHeightError: height_error_meter.max_error(state: state, mesh: mesh),
          protectedCrossingCount: coverage < 1.0 ? 1 : 0,
          protectedCrossingSeverity: coverage < 1.0 ? 'degraded' : 'none',
          hardViolationCounts: anchor_violation_counts(feature_geometry, mesh),
          anchorHitDistances: anchor_hit_distances(feature_geometry, mesh),
          topologyChecks: topology,
          topologyResiduals: topology_residuals(topology),
          firmResidualsByRole: firm_residuals_by_role(feature_geometry, coverage),
      ...
      def result_row(state:, feature_geometry:, normalized:, triangulation:, mesh:, metrics:,
                     budget_status:)
        {
          ...
          metrics: metrics,
          budgetStatus: budget_status,
          failureCategory: failure_category(feature_geometry, normalized, triangulation, metrics,
                                            budget_status),
          ...
          constraintCount: normalized.fetch(:segments).length +
          ...
          knownResiduals: [],
          limitations: limitations(feature_geometry, normalized, triangulation, budget_status),
      ...
      def failure_category(feature_geometry, normalized, triangulation, metrics, budget_status)
        ...
        unless metrics.fetch(:hardViolationCounts).empty? &&
               metrics.fetch(:protectedCrossingCount).zero?
      ...
      def dense_equivalent_face_count(state)
        (columns(state) - 1) * (rows(state) - 1) * 2
      end
