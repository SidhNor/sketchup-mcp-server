...
- The task does not introduce broad semantic query behavior, generic mutation compatibility rules, or identity-preserving replacement flows.
...
1. Freeze the public mutation contract.
   ...
   - Pin the target-reference shape, `updated` / `refused` result posture, and representative refusal codes.
2. Add failing Ruby behavior coverage.
   - Add metadata-layer tests for allowed and disallowed mutations.
   - Add semantic-command tests for target resolution, operation boundaries, and updated serialization.
