...
require_relative '../../src/su_mcp/terrain/terrain_surface_state_builder'

class TerrainSurfaceStateBuilderTest < Minitest::Test
  def test_builds_flat_create_heightmap_state_from_grid_definition
    state = SU_MCP::Terrain::TerrainSurfaceStateBuilder.new.build_create_state(
      create_request,
      owner_transform_signature: 'matrix:owner'
    )

    assert_instance_of(SU_MCP::Terrain::HeightmapState, state)
  ...
  def test_builds_adopted_heightmap_state_with_source_summary
    sampled_source = {
      state_input: {
